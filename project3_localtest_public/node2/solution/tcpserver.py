...
class Server():
    def __init__(self, config_file):
        # Remember working directory so we can find files relative to it
    ...
    def listener(self):
        while self.remain_threads:
            ...
            if ptype == "REQUEST":
                ...
                if already:
                    # Duplicate REQUEST — just re-send COUNT if we have a session
                    continue
    ...
    def load_file(self, file_name):
        ...
        while len(received) < packet_num and not fin_seen and self.remain_threads:
            now = time.time()
